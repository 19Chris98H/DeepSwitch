uniform sampler2D vectorData;
uniform sampler2D scalarData;
uniform float alpha;
...
vec3 lic() {
    vec2 texelSize = 1.0 / vec2(250.0, 250.0);
    ...
    for (int i = 0; i < STEPS; i++) {
        vec2 flow = texture2D(vectorData, sampleUv).rg;
        sampleUv += normalize(flow) * texelSize * stepSize;

        float noiseValue = texture2D(tex, sampleUv).r;
...
void main() {
	if (licMode == 1)
	{
        vec4 vectorDataColor = texture(vectorData, vUv);
        float alpha = min(vectorDataColor.a, alpha);
		outColor = vec4(lic(), alpha);
	}
	else
	{
        vec4 scalarDataColor = texture(scalarData, vUv);
        float scalarDataValue = scalarDataColor.r;
		float alpha = min(scalarDataColor.a, alpha);
		outColor = vec4(getColor(scalarDataValue), alpha);
	}
}